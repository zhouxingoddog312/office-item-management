...
create_dirs:
	...
	mkdir -p $(DEB_ROOT_DIR)$(SYSTEM_BIN_DIR)
	mkdir -p $(DEB_ROOT_DIR)$(SYSTEM_DESKTOP_DIR)
	$(foreach size,$(ICON_SIZES), \
		mkdir -p $(DEB_ROOT_DIR)$(SYSTEM_ICON_ROOT)/$(size)/apps; \
	)
	mkdir -p $(DEB_ROOT_DIR)$(SYSTEM_CONFIG_DIR)
#处理脚本模板（替换占位符）
#将postinst.template和postrm.template转换为postinst和postrm
process_scripts:
	@echo "===处理postinst/postrm/control/oim.desktop脚本（替换变量占位符）==="
#检查模板文件是否存在
	...
	if [ ! -f "$(DESKTOP_TEMPLATE)" ];\
	then \
		echo "错误：未找到oim.desktop模板文件$(DESKTOP_TEMPLATE)" >&2; \
		exit 1; \
	fi
#复制模板并替换占位符（生成最终postinst）
	...
	sed -i "s#{{SYSTEM_ICON_ROOT}}#$(SYSTEM_ICON_ROOT)#g" $(POSTINST_TARGET)
	sed -i "s#{{SYSTEM_DESKTOP_DIR}}#$(SYSTEM_DESKTOP_DIR)#g" $(POSTINST_TARGET)
	sed -i "s#{{SYSTEM_BIN_DIR}}#$(SYSTEM_BIN_DIR)#g" $(POSTINST_TARGET)
	sed -i "s#{{SYSTEM_CONFIG_DIR}}#$(SYSTEM_CONFIG_DIR)#g" $(POSTINST_TARGET)
#复制模板并替换占位符（生成最终postrm）
	...
	sed -i "s#{{SYSTEM_DESKTOP_DIR}}#$(SYSTEM_DESKTOP_DIR)#g" $(POSTRM_TARGET)
	sed -i "s#{{SYSTEM_BIN_DIR}}#$(SYSTEM_BIN_DIR)#g" $(POSTRM_TARGET)
	sed -i "s#{{SYSTEM_CONFIG_DIR}}#$(SYSTEM_CONFIG_DIR)#g" $(POSTRM_TARGET)
...
#复制模板并替换占位符（生成最终oim.desktop）
	cp -f $(DESKTOP_TEMPLATE) $(DESKTOP_TARGET)
	sed -i "s/{{VERSION}}/$(VERSION)/g" $(DESKTOP_TARGET)
	sed -i "s/{{MAIN_PROG_TARGET}}/$(MAIN_PROG_TARGET)/g" $(DESKTOP_TARGET)
	sed -i "s/{{DEB_NAME}}/$(DEB_NAME)/g" $(DESKTOP_TARGET)
	sed -i "s#{{SYSTEM_BIN_DIR}}#$(SYSTEM_BIN_DIR)#g" $(DESKTOP_TARGET)
	sed -i "s/{{ICON_TARGET_NAME}}/$(ICON_TARGET_NAME)/g" $(DESKTOP_TARGET)
...
#复制主程序到/usr/local/bin
	@if [ -f "$(MAIN_PROG_SRC)" ];\
	then \
		cp -v $(MAIN_PROG_SRC) $(DEB_ROOT_DIR)$(SYSTEM_BIN_DIR)/$(MAIN_PROG_TARGET);\
	else \
...
#复制库文件到/usr/local/bin
	@if [ -f "$(LIB_FILE_SRC)" ];\
	then \
		cp -v $(LIB_FILE_SRC) $(DEB_ROOT_DIR)$(SYSTEM_BIN_DIR)/$(notdir $(LIB_FILE_SRC));\
	else \
...
#复制图标文件到/usr/share/icons/hicolor（支持含尺寸子串的命名，如 icon_16x16.png）
	@$(foreach size,$(ICON_SIZES), \
		...
		if [ -n "$$icon_file" ];\
		then \
			cp -v "$$icon_file" $(DEB_ROOT_DIR)$(SYSTEM_ICON_ROOT)/$(size)/apps/$(ICON_TARGET_NAME);\
		else \
			echo "警告：未找到尺寸为$(size)的图标，跳过该尺寸";\
		fi;\
	)
#拷贝配置文件到/usr/local/etc/oim/
	@if [ -f "$(CONFIG_FILE)" ];\
	then \
		cp -v $(CONFIG_FILE) $(DEB_ROOT_DIR)$(SYSTEM_CONFIG_DIR)/$(notdir $(CONFIG_FILE));\
	else \
		echo "错误：配置文件$(CONFIG_FILE)不存在！" >&2 && exit 1;\
	fi
#设置文件权限主程序及库文件755,桌面文件644,图标文件644,control文件644,postinst等文件755
set_permissions:
	@echo "===设置文件权限==="
#主程序权限
	chmod 755 $(DEB_ROOT_DIR)$(SYSTEM_BIN_DIR)/$(MAIN_PROG_TARGET)
#库文件权限
...
#配置文件权限
	chmod 644 $(DEB_ROOT_DIR)$(SYSTEM_CONFIG_DIR)/$(notdir $(CONFIG_FILE))
#其余文件权限
...
distclean:clean
	@echo "===彻底清理打包文件==="
#删除主程序
	rm -fv $(DEB_ROOT_DIR)$(SYSTEM_BIN_DIR)/$(MAIN_PROG_TARGET)
#删除库文件
...
#删除配置文件
	rm -fv $(DEB_ROOT_DIR)$(SYSTEM_CONFIG_DIR)/$(notdir $(CONFIG_FILE))
#删除postinst
